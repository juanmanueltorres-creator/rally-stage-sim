...
function historicalPayload(year: number, index: number) {
  return {
    elevation: 120,
    hourly: {
      time: [`${year}-09-11T09:00`],
      temperature_2m: [10 + index * 2],
  ...
  const result = await fetchHistoricalReference(
    ...
    async (url) => {
      ...
      return {
        ok: true,
        async json() {
          return historicalPayload(year, index)
        },
      }
    },
  )
...
test('fetchHistoricalReference tolerates failed years when at least three yearly datasets remain available', async () => {
  const requestedYears: number[] = []
...
test('fetchHistoricalReference fails closed only after all five attempts when fewer than three yearly datasets load', async () => {
  const requestedYears: number[] = []
  ...
  await assert.rejects(
    ...
    /at least three historical years/i,
  )
